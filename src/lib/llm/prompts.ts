...
- クリック誘引: この記事ならではの独自性（他の記事には無い切り口・具体的な状況・意外な着眼点・当事者ならではの気づき）を一つ選び、それを読者が「続きを読みたい」と感じる形で提示する。ありふれた一般論ではなく、この1本を開く理由になる要素を出すこと。ただし煽り表現には頼らない（下記「禁止」参照）。
...
export function buildSingleCurationPrompt(input: CurationInput, feedback?: string): string {
  const feedbackBlock = feedback
    ? `\n【再生成フィードバック】\n以下の理由で直前のアンカーが検証に失敗しました。同じタイトル・抜粋で、タイトルに無い視点を足した「問いを立てる節」にしなさい。\n<feedback>\n${feedback}\n</feedback>\n`
    : "";
