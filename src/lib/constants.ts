...
// ── フィード表示 ──────────────────────────────────────────────
/** フィード初期表示・「もっと見る」1回あたりの増分カード数。 */
export const FEED_PAGE_SIZE = 24;
/**
 * `?count=` で指定できる表示件数の上限（サニティキャップ）。
 * 実件数（2026-09時点で公開ブログ134件）を大きく超えて設定しており、
 * 通常の「もっと見る」連打では到達しない。`?count=999999` のような
 * 過大なクエリでDBに不要な負荷をかけることだけを防ぐための上限であり、
 * 供給量そのものを制限する意図はない。
 */
export const FEED_PAGE_SIZE_MAX = 240;
