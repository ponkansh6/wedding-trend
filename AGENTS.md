...
本プロジェクトは**記事本文を一切生成しない**独立したキュレーションメディアである。
法務制約の詳細は **spec.md §10** を参照。緩めてはならない。

（2026-08-29: 「集約レベルの中立性」＝単一ホストがフィードを占有しない保証は
運用ポリシーから外した。ホスト別公開シェア上限 `HOST_DAILY_SHARE_MAX` は廃止。
ただし記事本文の非生成・非永続化、逐語タイトル、アクセス規律は
法務不変部分として維持する。詳細は spec.md §11 項4。）

（2026-08-31 訂正: 上記に挙げていた「語彙的接地検証」は法務不変部分ではない。
2026-08-29 のゲート緩和（第3段）で `checkAnchorGrounding` は `validateTopicAnchor`
から撤廃済みであり、現在は呼び出し元のない休眠関数である。同様に、根拠文の数値禁止を
zod `refine` が機械的に拒否するという spec の記述も実装と一致しないため、
spec.md §10 第3項を実態に合わせて訂正した。現在アンカーに対する機械的検証は
長さ下限（6字）と個人識別情報 denylist の2点のみである。）
