...
- 作成日: 2026-08-31
- 状態: **完了（2026-08-31）。** Stage 1/2/3/4/5/6/7/8/9 をすべて処理した。
  Stage 5（S7 墓碑化）は調査により対象ゼロでクローズ。
  Stage 9（S9）の定数分離は、プランの「JSON に出して zod で parse」から
  **別 TS モジュールへの切り出しに変更した**（オーナー判断）。理由は下記。

  S8 / S9 完了時の記録（2026-08-31）:
  - S8: 「RAM 7.4GiB のため直列」という前提を実測で検証したところ成り立っていなかった。
    568 テストのピーク RSS は約 415MB（2桁の乖離）。さらに支配的だったのは並列化ではなく
    environment で、**happy-dom を必要とするテストは1つも無かった**（構築に毎回約20秒）。
    node 環境化と4ワーカー並列で 30.91s → 11.80s（62%短縮）。
    並列化で顕在化した feed-order-parity のタイムアウト脆弱性も併せて修正した。
  - S9: `HOST_ALLOWLIST` の JSON 化は**採用しなかった**。`articlePathPatterns` は RegExp
    リテラルであり、文字列化するとエスケープ誤りで静かに壊れる。さらに各エントリには
    「記事と口コミ投稿をどう区別するか」等の根拠が長いコメントで書かれており、JSON では
    失われる。これは UGC 混入を構造的に防ぐホワイトリストであり、本プラン自身の禁止事項
    （「段数削減はシンプル化ではなく防御の削減」）に抵触する。別 TS モジュール
    （`src/lib/sources/host-allowlist.ts`）へ切り出すことで、プランが挙げる利点
    （ホスト追加がロジック変更と区別できる差分になる）は同じく得られる。
    `constants.ts` は 468行 → 377行。
  - S9: `shared_plan/` は 18 → 10 ファイル。決着したものを `archive/` へ移した。
    **状態行の自己申告は信用できなかった**（13 と 14 は実装済みなのに「未着手」のまま
    放置されていた）ため、実装の裏付けで判断し、移設基準を `archive/README.md` に記録した。
