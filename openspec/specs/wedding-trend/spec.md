...
- **トピックアンカー（`topicAnchor`、`post_rationales.topic_anchor`）は 40字以内**とし、記事が扱う**「具体的な判断・場面・選択肢」**を本文語句で名指しするものとする。結論・結果・具体的数値を開示してはならない（可: 「席次表づくりで迷った優先順位」／不可: 「席次表は親族優先で決めた」「持ち込み料〇万円が免除された」）。語句は原則として本文抜粋に実在するもののみとし、機械的検証（`checkAnchorGrounding`）で接地を確認する。ただし、接続詞やフレーミングを形成する許可リスト（`CONNECTOR_ALLOWLIST`：理由・経緯・きっかけ・選び方等）に属する用語は逐語一致の要件から除外する。これによりコンテンツ名詞のハルシネーション（非実在の捏造）を厳格に防ぎつつ、クリック率を向上させる自然な表現を可能にする。- **改定の経緯（2026-08-27, shared_plan/15）**: §10-3 のトピックアンカー設計を刷新。結論の開示禁止という消極的制約から、「記事が扱う具体的な判断・場面・選択肢を本文語句で名指しする（体言止め）」という積極的方針へ緩和し、クリック率向上と信頼性維持を両立させる。接地検証（`checkAnchorGrounding`）はトークンレベル＋接続詞許可リスト方式へ移行し、数字・固有名詞の禁止は維持する。
